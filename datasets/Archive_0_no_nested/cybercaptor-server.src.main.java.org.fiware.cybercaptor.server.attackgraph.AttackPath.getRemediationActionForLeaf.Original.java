...
if (sourceHost == null || !sourceHost.equals(currentAttackerPathHost)) {
remediation.getPossibleMachines().add(currentAttackerPathHost);
}
}
}
...
if (command.params[1].equals("internet")) {
remediation.getPossibleMachines().add(currentAttackerPathHost);
}
 else {
InformationSystemHost destinationHost=topology.existingMachineByNameOrIPAddress(command.params[1]);
if (destinationHost == null || !destinationHost.equals(currentAttackerPathHost)) {
remediation.getPossibleMachines().add(currentAttackerPathHost);
}
}
}
