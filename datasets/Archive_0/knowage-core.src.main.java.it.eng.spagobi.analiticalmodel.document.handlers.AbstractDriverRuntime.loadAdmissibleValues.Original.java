...
protected void loadAdmissibleValues(AbstractDriver driver,AbstractBIResourceRuntime dum){
  ...
  try {
    ...
    if (retrieveAdmissibleValue) {
      ...
      if (isFromCross) {
        logger.debug("Parameter value for parameter " + driver.getParameterUrlName() + " is retrieved from cross so it is necessary to retrieve description from admissible values");
          ...
          if (value != null) {
            ...
            for (Iterator iterator2=admissibleValues.iterator(); iterator2.hasNext() && !found; ) {
              ...
              if (valueD != null && valueD.equals(value)) {
                String description=map.get("description") != null && map.get("description") instanceof String ? map.get("description").toString() : null;
                if (description != null) {
