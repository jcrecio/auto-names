...
class OpenAiModel():
    ...
    def predict_extraction(self, original_code, extraction_code, method_to_compute, version = "v4"):
        prediction = self.chat(predict_extraction_versions[version](original_code, extraction_code, method_to_compute))
    ...
    def predict(self, input_code, method_name=None):
        if (method_name == None):
            prediction = self.chat("""Given the following java code, suggest a name for the method without taking into account the current method name and provide only the method name in the answer without any additional data, symbols or spaces. (Do not provide more than one method name, and always use java standard and notation). This is the code: """ + input_code)
            time.sleep(sleep_api)
            return prediction.replace('()', '')
        prediction = self.chat("""Given the following java code, suggest a name for the method : """ + method_name + ' without taking into account its current method name and provide only the method name in the answer without any additional data, symbols or spaces. (Do not provide more than one method name). This is the code: ' 
                    + ' code: ' + input_code)
        time.sleep(sleep_api)
        return prediction.replace('()', '')
